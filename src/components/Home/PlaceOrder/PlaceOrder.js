...
const PlaceOrder = () => {
    ...
    const onSubmit = data => {
        data.order = tours._id;
        data.status = 'pending';

        const url = `https://dreadful-eyeballs-21271.herokuapp.com/placeOrder`;
    ...
    useEffect(() => {
        const url = `https://dreadful-eyeballs-21271.herokuapp.com/tours/${tourId}`;
        ...
        <Container>
            <Row className='mt-4 '>
                <Col lg={4}>
                        ...
                        <Card className='p-0'>
                            <Card.Img variant="top" className='img-fluid' src={tours?.image} />
                            <Card.Body>
                                <Card.Title>{tours?.title}</Card.Title>
                                <Card.Text><small className='text-muted'>{tours?.description}</small></Card.Text>
                                <Card.Text><small className='text-muted'>Price: ${tours.price}</small></Card.Text>
                            </Card.Body>
                        </Card> 
                </Col>
                <Col lg={8}>
